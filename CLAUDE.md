...
### مراحل لاحقة (بعد MVP)
- [x] تحليل AI موسّع — قوائم مالية: عميل `src/lib/fmp.js` (تعريف + نسب TTM + آخر قائمة دخل، كاش يوم، `VITE_FMP_API_KEY`) يُضاف لسياق التحليل. مقارنات: صفحة `/compare` (رابط "مقارنة") تختار ورقتين من الكون وترسل `mode: 'compare'` لـ `api/analyze.js` ببرومبت مقارنة منفصل (مالية/شرعية/خلاصة). `AnalysisText` مكوّن مشترك. البرومبت الفردي أيضاً يستقبل القوائم المالية الآن.
- [x] الصكوك والصناديق الإسلامية (إدخال يدوي منظم) — `INSTRUMENT_FIELDS` + كائن `details` على الورقة، قسم شرطي في النموذج و`InstrumentDetailsCard` في صفحة الورقة. seed: صك IsDB.
- [x] تقويم اقتصادي + heatmap قطاعات — صفحة `/markets` (رابط "الأسواق"): `SectorHeatmap` (stock-heatmap) + `EconomicCalendar` (events widget، دول: US/TR/GCC/EU).
- [x] حاسبة زكاة + حاسبة تطهير أرباح — صفحة `/tools` (رابط "الأدوات" في الناف). الدوال النقية في `src/lib/zakat.js`: زكاة بنصاب 85 جرام ذهب (سعر الجرام إدخال يدوي) ومعدل هجري 2.5% / ميلادي 2.5775%، مدخلاتها تُحفظ في localStorage. التطهير = التوزيعات × نسبة الدخل غير المتوافق، واختيار ورقة من الكون الحلال يملأ نسبتها تلقائياً (قابلة للتعديل). تنويه فقهي واضح في الواجهتين.
## اتجاه التصميم
...
- [x] قرار وربط API الفرز الشرعي — **القرار: Halal Terminal API** (بموافقة المالك بعد مقارنة الخيارات). طبقة محايدة `src/lib/screening/` (orchestrator يتخطى المفحوص خلال 30 يوم والأنواع غير القابلة للفرز، تسلسلي مع مهلة 400ms) + adapter `halalterminal.js` (تطبيع مرن للحالات والنسب). زر "تحديث الفرز" في `/universe` مع تقدم وملخص. **استيراد كون جاهز** (`ImportUniverse` في `/universe`): يجلب أسهماً متوافقة مفحوصة عبر `GET /api/database/search` (فلترة بالقطاع + حد)، يطبّعها لأوراق كون حلال ويدمجها (تخطّي المكرر عبر tvSymbol). يتطلب `VITE_HALALTERMINAL_API_KEY`. ⚠ المسارات مبنية على وصف الـ API المعلن (الفرز الفردي `/api/screen/{symbol}`، التصفّح `/api/database/search`) دون رؤية التوثيق الكامل (محجوب) وأشكال الاستجابة مُطبّعة بمرونة — تحقق من المسارات/الطرق/الحقول أول تشغيل بمفتاح حقيقي.
